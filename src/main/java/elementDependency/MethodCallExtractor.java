...
public class MethodCallExtractor extends DependencyExtractor{

	public MethodCallExtractor(Repository repository) {
		super(repository);
	}
	
	
	public boolean excludeSibling(CFGNode cfgnode, JavaClass srcclass) {
		return ((CFGMethodEntry)cfgnode).getJavaMethod().getDeclaringClass().equals(srcclass);
	}
...
//					System.out.println("src: "+((CFGMethodEntry)edge.getSrcNode()).getJavaMethod().getDeclaringClass());
					//callee is a method, exclude sibling callee
					CFGNode dstNode = edge.getDstNode();
					if(dstNode.getClass().equals(CFGMethodEntry.class) && !excludeSibling(dstNode, jclass)) {
						callees.add(((CFGMethodEntry)dstNode).getJavaMethod());
					}
				}
		...
		for(ClassLevelCallGraph clcg:callOutGraph) {
			...
			for(JavaElement src:clcg.getMethodCall().keySet()) {
				for(JavaElement dst:clcg.getMethodCall().get(src)) {
					...
					if(dst.getClass().equals(JavaMethod.class)) {
						JavaClass curclass = ((JavaMethod)dst).getDeclaringClass();
